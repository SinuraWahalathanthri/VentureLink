  ...
  try {
    ...
    if (!snap.exists()) {
      await redirectToLogin(
        "No Firestore user profile found for this account.",
        true,
      );
      return;
    }
    ...
    await redirectToLogin(
      `Dashboard auth bootstrap failed: ${err?.message || "Unknown error"}`,
      true,
    );
  }
});
...
function hydrateUserUI() {
  ...
  const firstName =
    ...
    currentUserProfile.displayName?.split(" ")[0] ||
    "Owner";
...
function updateCfStepUI(step) {
  [1, 2, 3, 4].forEach((i) => {
    ...
    const label =
      document.getElementById(`cfsl${i}`) ||
      document.getElementById(`cfsl${i}`); // some labels only 2..4
    if (!circle) return;
...
function renderBizCardsCached() {
  ...
  const filter =
    activeFilterEl?.dataset?.status ||
    activeFilterEl?.getAttribute("data-status") ||
    "all";
...
export function openPayModal(campaignId) {
  currentPayingCampaign =
    campaignsCache.find(
      (c) => c.id === campaignId || c.businessName === campaignId,
    ) || null;
  if (!currentPayingCampaign) return;
  if (currentPayingCampaign.status !== "accepted") {
    showToast(
      "Only accepted campaigns can be published with payment.",
      "error",
    );
    return;
  }
  const nameEl = document.getElementById("payModalCampaignName");
  if (nameEl)
    nameEl.textContent = currentPayingCampaign.businessName || "your campaign";
  openModal("payModal");
}
...
function populateListingInvoiceModal(orderId, campaign) {
  ...
  const invOwner = document.getElementById("invOwner");
  const invOwnerContact = document.getElementById("invOwnerContact");
  const invOwnerLocation = document.getElementById("invOwnerLocation");
  const invBiz = document.getElementById("invBiz");
  const invBizMeta = document.getElementById("invBizMeta");
  const invMethod = document.getElementById("invMethod");
  const invTxn = document.getElementById("invTxn");

  if (invNo) invNo.textContent = `Invoice #${orderId}`;
  if (invDate) invDate.textContent = when;
  if (invOwner) invOwner.textContent = ownerName;
  if (invOwnerContact) invOwnerContact.textContent = `${email} · ${mobile}`;
  if (invOwnerLocation) invOwnerLocation.textContent = loc;
  if (invBiz) invBiz.textContent = campaign?.businessName || "—";
  if (invBizMeta) {
    const ind = campaign?.industry || "—";
    const cityDist =
      [campaign?.city, campaign?.district].filter(Boolean).join(", ") ||
      "Sri Lanka";
    invBizMeta.textContent = `${ind} · ${cityDist}`;
  }
  if (invMethod) invMethod.textContent = "PayHere (card / wallet)";
  if (invTxn) invTxn.textContent = orderId;
}

async function applyPublishAfterPayHere(orderId) {
  if (!currentPayingCampaign?.id || !currentUser) return;
  ...
  const data = snap.data();
  if (String(data.ownerUid) !== String(currentUser.uid)) {
    showToast("You cannot update this campaign.", "error");
    return;
  }

  const merged = { id: currentPayingCampaign.id, ...data };

  if (data.status === "published") {
    populateListingInvoiceModal(orderId, merged);
    closeModal("payModal");
    openModal("invoiceModal");
    showToast("Campaign is already published. Receipt opened.", "success");
    return;
  }

  if (data.status !== "accepted") {
    showToast(
      "This campaign is no longer awaiting payment. Refresh the page.",
      "error",
    );
    return;
  }
  ...
  await setDoc(
    campaignRef,
    {
      ...
      listingFeePaidAt: serverTimestamp(),
      listingFeeAmountLkr: 1500,
      listingFeePayHereOrderId: orderId,
...
export function startPayHereListingPayment() {
  ...
  if (currentPayingCampaign.status !== "accepted") {
    showToast("This campaign is not awaiting listing payment.", "error");
    return;
  }
  if (typeof payhere === "undefined" || !payhere.startPayment) {
    showToast(
      "PayHere script did not load. Check your connection and try again.",
      "error",
    );
    return;
  }

  var orderId = "PUBLISH_" + currentPayingCampaign.id + "_" + Date.now();
  var amount = "1500.00";
  ...
  setButtonLoading("startPayHereBtn", true);
  ...
  request.onreadystatechange = function () {
    if (request.readyState === 4) {
      ...
      setButtonLoading("startPayHereBtn", false);
      ...
      if (request.status === 200) {
        ...
        try {
          ...
          console.error("Invalid JSON from buyNowProcess.php", e);
        ...
        console.log("Merchant ID:", obj.merchant_id);
        console.log("Hash:", obj.hash);
        console.log("==================================");

        payhere.onCompleted = function (completedOrderId) {
          console.log("✅ PAYMENT COMPLETED");
          console.log("Order ID:", completedOrderId);
          void (async () => {
            try {
              await applyPublishAfterPayHere(completedOrderId);
            } catch (e) {
              console.error("applyPublishAfterPayHere", e);
              showToast(
                "Payment completed but saving failed: " +
        ...
        payhere.onError = function (error) {
          ...
          console.log(error);
          showToast(
            "PayHere error: " + (error != null ? String(error) : "Unknown"),
            "error",
          );
        };
        ...
        console.log("🔑 HASH BEING SENT:", obj.hash);

        payhere.startPayment(payment);
      } else {
        showToast(
          "Could not get payment hash (HTTP " + request.status + ").",
          "error",
        );
      }
    }
  ...
  request.open(
    "GET",
    "http://localhost:8000/buyNowProcess.php?order_id=" +
      orderId +
      "&amount=" +
      amount,
    true,
...
function fmtCompactLkrNumber(n) {
  const v = Number(n) || 0;
  if (v >= 1_000_000_000)
    return (v / 1_000_000_000).toFixed(v % 1_000_000_000 === 0 ? 0 : 2) + "B";
  if (v >= 1_000_000)
...
function committedCell(c) {
  ...
  const comStr =
    com >= 1_000_000
      ? fmtCompactLkrNumber(com) + "M"
      : fmtLKR(com).replace("LKR ", "");
  return `${comStr} (${pct}%)`;
}
...
export function renderDashboardOverview() {
  ...
  if (elCampMeta) {
    ...
    if (counts.changes)
      parts.push(`<strong>${counts.changes}</strong> changes req.`);
    if (counts.rejected)
      parts.push(`<strong>${counts.rejected}</strong> rejected`);
  ...
  if (elGoal)
    elGoal.textContent = totalGoal ? fmtCompactLkrNumber(totalGoal) : "0";
  ...
  if (elCom)
    elCom.textContent = totalCommitted
      ? fmtCompactLkrNumber(totalCommitted)
      : "0";
  if (elComMeta) {
    const pct = totalGoal
      ? Math.round((totalCommitted / totalGoal) * 1000) / 10
      : 0;
...
export function renderPendingView() {
  ...
  if (accepted.length && banner && titleEl && payBtn) {
    const latest = accepted.reduce((a, b) =>
      tsToMs(a.createdAt) >= tsToMs(b.createdAt) ? a : b,
    );
  ...
  const sorted = [...campaignsCache].sort(
...
export function renderInvestors() {
  ...
  const largest = data.length
    ? Math.max(...data.map((i) => i.amountLkr || 0))
    : 0;
...
export function openInvDetail(id, event) {
  ...
  document.getElementById("invDetailAmount").textContent = fmtLKR(
    inv.amountLkr || 0,
  );
  ...
  if (mailBtn)
    mailBtn.onclick = () => sendMail(inv.id, { stopPropagation() {} });
